...
class GroupController extends Controller{
    ...
    public function search(Request $request)
    {
        ...
        $result = DB::table('ki_admin_group')->where('group_name', 'like', '%' . $group_name . '%')->where('cut_off', '!=', '1')->where('domain_id', $domain_id)->get()->toArray();
    ...
    public function supplyInfo($id){
        $result=$this->groupServices->supplyInfo($id);//获得组内的人机
        return response()->json(['status' => 'S', 'code' => '200', 'message' => $result]);
    }
